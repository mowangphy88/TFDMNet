...
@tf.function
def train_step(model, input_Real, input_Imaginary, labels, optimizer):
    with tf.GradientTape() as tape:
        prediction = model(input_Real, input_Imaginary, training=True)
        loss = cross_entropy_batch(labels, prediction, label_smoothing=c.label_smoothing)
        # l2 = l2_loss(model)
        # loss = ce + l2
...
def train(model, data_iterator, optimizer, log_file):
    ...
    for i in tqdm(range(c.iterations_per_epoch)):
        input_Real, input_Imaginary, labels = data_iterator.next()
        ce, prediction = train_step(model, input_Real, input_Imaginary, labels, optimizer)
        correct_num = correct_num_batch(labels, prediction)

        sum_ce += ce * c.batch_size
        sum_correct_num += correct_num
        print('ce: {:.4f}, accuracy: {:.4f}'.format(ce,
...
if __name__ == '__main__':
    ...
    # load data
    train_data_iterator = train_iterator(batch_size=c.batch_size)
    it = train_data_iterator.__iter__()

    # get model
    # model = ResNet(50)
    # model = ResNet_v2(50)
    model = VGG16_CEMNet_ImageNet(c.batch_size)
