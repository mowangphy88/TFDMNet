...
def load_image(image_path, label, augment=False, crop_10=False):
    ...
    # TODO: Transfer inputs into the frequency domain
    # input_time = tf.cast(image, dtype=tf.complex64) # N-H-W-C           
    # input_time = tf.transpose(input_time, [2, 0, 1])
    # input_freq = tf.signal.fft2d(input_time)
    # input_freq = tf.transpose(input_freq, [1, 2, 0])
        
    # input_Real = tf.math.real(input_freq)
    # input_Imaginary = tf.math.imag(input_freq)
    imDepth = image.shape[2]
    input_Real = np.zeros_like(image, dtype=np.float)
    input_Imaginary = np.zeros_like(image, dtype=np.float)
    for iChannel in range(imDepth):
        fft2 = np.fft.fft2(image[:, :, iChannel])
        input_Real[:, :, iChannel] = fft2.real
        input_Imaginary[:, :, iChannel] = fft2.imag

    label_one_hot = np.zeros(c.category_num)
    label_one_hot[label] = 1.0

    return input_Real, input_Imaginary, label_one_hot
...
def train_iterator(batch_size, list_path=c.train_list_path):
    ...
    dataset = dataset.map(lambda x, y: tf.py_function(load_image, inp=[x, y, True, False], Tout=[tf.float32, tf.float32, tf.float32]),
...
def test_iterator(batch_size, list_path=c.test_list_path):
    ...
    dataset = tf.data.Dataset.from_tensor_slices((images, labels))
    dataset = dataset.map(lambda x, y: tf.py_function(load_image, inp=[x, y, False, False], Tout=[tf.float32, tf.float32, tf.float32]),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
    dataset = dataset.batch(batch_size)
    # it = dataset.__iter__()
    return dataset
...
def test_10_crop_iterator(list_path=c.test_list_path):
    images, labels = load_list(list_path, c.test_data_path)
    dataset = tf.data.Dataset.from_tensor_slices((images, labels))
    dataset = dataset.map(lambda x, y: tf.py_function(load_image, inp=[x, y, False, True], Tout=[tf.float32, tf.float32, tf.float32]),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
    # it = dataset.__iter__()
    return dataset
