...
class ComplexDotLayer(tf.keras.layers.Layer):
    def __init__(self, featureSize, outChannels, filterSize, use_bias=False):
        ...
        self.initMethod = 0  
        self.convfilterSize = filterSize
        self.convfilterSizeIdentitySize = filterSize
...
class ComplexPoolLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, pooling_window_size, featureSize, strides):
        super(ComplexPoolLayer, self).__init__()  # using average pooling
        # weight_decay = 5e-4
        self.pooling_window_size = pooling_window_size
        self.featureSize = featureSize
        self.pool = tf.keras.layers.MaxPooling2D((self.pooling_window_size, self.pooling_window_size), strides=strides)
...
class AlexNet_MixtureNet_ImageNet(tf.keras.models.Model):
    ...
    def __init__(self, batchSize):
        ...
        """
        super(AlexNet_MixtureNet_ImageNet, self).__init__()
        ...
        self.out_channel = [96, 256, 384, 256]
        ...
        # Block 2
        self.complex2 = ComplexDotLayer([self.batchSize, 27, 27, self.out_channel[0]], self.out_channel[1], 5, use_bias=False)
        self.drop2 = ComplexDropoutLayer_approx(self.droprate2)
               
        self.pool2 = ComplexPoolLayer(3, [self.batchSize, 27, 27, self.out_channel[1]], 2)

        
        # Block 3
        self.complex3 = ComplexDotLayer([self.batchSize, 13, 13, self.out_channel[1]], self.out_channel[2], 3, use_bias=False)
        self.drop3 = ComplexDropoutLayer_approx(self.droprate2)
        
        self.complex4 = ComplexDotLayer([self.batchSize, 13, 13, self.out_channel[2]], self.out_channel[2], 3, use_bias=False)
        self.drop4 = ComplexDropoutLayer_approx(self.droprate2)
        
        self.complex5 = ComplexDotLayer([self.batchSize, 13, 13, self.out_channel[2]], self.out_channel[3], 3, use_bias=False)
        self.drop5 = ComplexDropoutLayer_approx(self.droprate2)
               
        self.pool3 = ComplexPoolLayer(3, [self.batchSize, 13, 13, self.out_channel[3]], 2) # 7# 6
        ...
        self.fc1 = complexfcLayer(2048)
        self.drop6 = ComplexDropoutLayer_approx_fc(self.droprate3)
        self.fc2 = complexfcLayer(2048)
        self.drop7 = ComplexDropoutLayer_approx_fc(self.droprate3)
    ...
    def call(self, inputs, training=None):
        ...
        # Block 2
        x_real, x_imaginary = self.complex2([x_real, x_imaginary], training=training, isFeatFix=False)  # include bnorm and relu
        ...
        # Block 3
        x_real, x_imaginary = self.complex3([x_real, x_imaginary], training=training, isFeatFix=False)  # include bnorm and relu
        x_real, x_imaginary = self.drop3([x_real, x_imaginary], training=training)

        x_real, x_imaginary = self.complex4([x_real, x_imaginary], training=training, isFeatFix=False)  # include bnorm and relu
        x_real, x_imaginary = self.drop4([x_real, x_imaginary], training=training)
        
        x_real, x_imaginary = self.complex5([x_real, x_imaginary], training=training, isFeatFix=False)  # include bnorm and relu
